...
<script type="text/javascript">
	...
	{
		var url = '/search_city.php?type=name_company&company=<?=$arResult["AGENT"]["ID"];?>&type_company=<?=$arResult['TYPE_CLIENT_RECIPIENTS'];?>&branch=<?=intval($arResult["CURRENT_BRANCH"]);?>';
...
<?
 ...
 //	 print_r ($arResult['INVOICE']['PROPERTY_INNER_NUMBER_CLAIM_VALUE']);
 // echo "</pre> -->";
...
if ($arResult['OPEN'])
{
	...
	<form action="" method="post" name="curform" class="form-vertical">
                                ...
                                <tr id="addr<?=$k;?>">
                                    ...
                                    <td>
                                        <select size="1" name="goods[<?=$k;?>][persentnds]" class="form-control" id="input-goods-persentnds-<?=$k;?>" onChange="CalcGoods('<?=$k;?>');">
                                            <option value="20"<?=((intval($v['PersentNDS']) == 20) && isset($v['PersentNDS'])) ? ' selected' : '';?>>20%</option>
        ...
        <div class="col-md-6">
            	<div class="form-group <?=$arResult["ERR_FIELDS"]["INSTRUCTIONS"];?>">
                    ...
                    <textarea name="INSTRUCTIONS" class="form-control" style="height:100px; resize:vertical;"><?=$arResult['INVOICE']['PROPERTY_INSTRUCTIONS_VALUE']['TEXT'];?></textarea>
                </div>
				<div class="form-group">
				  <label class="control-label">Внутренний номер накладной</label>
				  <input name="INNER_NUMBER_CLAIM" type="text" class="form-control" value="<?=$arResult['INVOICE']['PROPERTY_INNER_NUMBER_CLAIM_VALUE'];?>">
				</div>
            </div>
        </div>
        <div class="row">
	...
	{
		?>
        <div class="alert alert-dismissable alert-danger">Накладная не найдена. <a href="/index.php?mode=list">Вернуться к списку накладных</a>.</div>
        <?
	}
}
?>
